  ...
  for (const f of fs.readdirSync(dir).filter(f => f.endsWith('Demo.jsx')).sort()) {
    ...
    try {
      ...
      results.push({
        ...
        /* ---- 以下是取用 CLI 要的字段，全部从源码 AST 推导，不信文档 ---- */

        // 本地实际路径。上面那个 source 字段存的是上游仓库里的路径，照它找文件 100% 落空
        localPath: path.join('components', relSrc).split(path.sep).join('/'),
        // 真实依赖。跟上面的 dependencies（上游文档抄来的）实测 139 个里有 11 个对不上
        realDeps: realDependencies(source),
        // 同目录资源，取用时必须跟着走（只有 Lanyard 的 card.glb / lanyard.png）
        assets: importedAssets(source),
        // 绝对路径引用的 public 资源，取用时要拷进目标项目并重写路径（只有 FluidGlass 的 6 个）
        publicAssets: publicAssets(source),
        // 硬编码的外链。开发机联网时不报错，打包进 Electron 交给离线用户才炸
        remoteUrls: remoteUrls(source),
        // 没有 default export 的要用具名导入（139 个里只有 GridScan）
        namedExportOnly: !hasDefaultExport(source),
        // 带 Tailwind 类的组件，宿主没装 Tailwind 会静默无样式 —— 不报错，就是布局全塌
        usesTailwind:
          /className=["'`][^"'`]*\b(?:flex|grid|absolute|relative|w-|h-|p-|m-|text-|bg-|rounded|border|inset-|z-)/.test(
            source
          ),
        // 用 useGLTF 的要连 public/draco/ 一起搬，并在入口 setDecoderPath
